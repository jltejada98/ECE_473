...
class Gaussian_Naive_Bayes():
    ...
    def std(self, x):
        ...
        summation = np.sum(square_difference, axis=0)
        std = np.sqrt((1.0/(len(x)-1))*summation) + 1e-7
    ...
    def predict(self, x):
        ...
        for sample in range(0,n):
            for c in range(0, num_class):
                prediction[sample][c] =  self.y_prior[0][c] + np.sum(np.log(self.calc_gaussian_dist(x[sample], self.mean_by_class[c], self.std_by_class[c])+ 1e-7))
